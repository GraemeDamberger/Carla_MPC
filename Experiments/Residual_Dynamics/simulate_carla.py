...
def simulate_carla(trial_num,log_dir):
    ...
    def cost_fun(M_u, X0, V, X_des, Y_des, leg,sys, Np, N, model_norm, Q):
        ...
        temp_X =  1 / scale_V * V * temp_X + V*M_d[:N]
        # temp_X+=X0[0]
        temp_Y =  1 / scale_V * V * temp_Y + V*M_d[N:]
        # temp_Y+=X0[1]
    ...
    # LMU
    LMU_Mem = LMU(N, Np * dt, dt)
    u_Mem = np.zeros((N))
    x_Mem = np.zeros((N))
    y_Mem = np.zeros((N))
    buffer = []  # Initialize buffer
    buffer_size = config['buffer_size']  # Set a buffer size
...
#    model_path = project_root / "Experiments" / "Normal" / "logs" / "run_2025-07-24_17-24-41" / "models" / "model_trial_0"
    ...
    optim = torch.optim.Adam(model_residual.parameters(), lr=online_lr, weight_decay=online_weight_decay)
    options = {
            'eps': 0.001, #0.001
            #    'maxiter': 5
        }
    ...
    try:
        for i in range(1, Steps):
            ...
            u_Mem = LMU_Mem.dynamics(u_Mem, U[0])
            x_Mem = LMU_Mem.dynamics(x_Mem, X[0, i])
            y_Mem = LMU_Mem.dynamics(y_Mem, X[1, i])

            U_data = np.copy(u_Mem)
            U_data = U_data.reshape(-1)
            U_data[1::2] *= -1
            X_data = np.copy(x_Mem)
            X_data = X_data.reshape(-1)
            X_data[1::2] *= -1
            Y_data = np.copy(y_Mem)
            Y_data = Y_data.reshape(-1)
            Y_data[1::2] *= -1

            X_data = leg.encode(leg.decode(X_data))
            ...
            if(i == window_steps+buffer_size):
                print(f"Online training turned on")
            if (i > window_steps):
                new_data = (M_u_tensor, M_x_tensor)
                buffer = add_to_buffer(buffer, new_data)

                if (i > window_steps + buffer_size):
                    replay_data = random.sample(buffer, min(len(buffer), batch_size))

                    input_data, target_data = zip(*replay_data)
                    input_data = torch.stack(input_data)
                    target_data = torch.stack(target_data)

                    prediction = model_norm(input_data)
                    res_target = target_data - prediction

...
def train(trial_num,log_dir):
    ...
    for epoch in range(num_epochs):
        ...
        # step scheduler on val loss so it reacts to overfitting/plateau
        scheduler.step(avg_val_loss)
        lr_log.append(optimizer.param_groups[0]['lr'])

        if (epoch + 1) % 25 == 0:
            print(f"[{epoch+1}/{num_epochs}] Train: {avg_train_loss:.6f} | Val: {avg_val_loss:.6f} | LR: {lr_log[-1]:.2e}")
    ...
    plt.grid(True, which="both", linestyle="--", linewidth=0.5)
    save_plot(log_dir, fig, f"learning_rate_{trial_num}")
